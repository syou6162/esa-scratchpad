...
fn post_text_separator_detected() {
    ...
    assert_eq!(issues[0].code, ValidationCode::Separator);
}

#[test]
fn post_text_separator_in_table_allowed() {
    let issues = validate_post_text("| col1 | col2 |\n| --- | --- |\n| a | b |");
    let separator_issues: Vec<_> = issues
        .iter()
        .filter(|i| i.code == ValidationCode::Separator)
        .collect();
...
fn post_text_bold_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::Bold);
}
...
fn post_text_heading_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::Heading);
}
...
fn post_text_fullwidth_colon_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::FullwidthColon);
}
...
fn post_text_fullwidth_paren_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::FullwidthParen);
}
...
fn post_text_leading_time_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::LeadingTime);
}
...
fn post_text_list_marker_hyphen_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::ListMarker);
}
...
fn post_text_multiple_violations() {
    let issues = validate_post_text("# 見出し\n---\n**太字**テスト");
    assert!(issues.len() >= 3);
    let codes: Vec<ValidationCode> = issues.iter().map(|i| i.code).collect();
    assert!(codes.contains(&ValidationCode::Separator));
    assert!(codes.contains(&ValidationCode::Bold));
    assert!(codes.contains(&ValidationCode::Heading));
}
...
fn post_text_time_in_middle_not_detected() {
    ...
    let time_issues: Vec<_> = issues
        .iter()
        .filter(|i| i.code == ValidationCode::LeadingTime)
        .collect();
...
fn post_text_hash_without_space_not_heading() {
    ...
    let heading_issues: Vec<_> = issues
        .iter()
        .filter(|i| i.code == ValidationCode::Heading)
        .collect();
...
fn title_slash_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::Slash);
}
...
fn title_halfwidth_space_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::Space);
}
...
fn title_leading_symbol_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::LeadingSymbol);
}
...
fn title_symbol_in_middle_ok() {
    ...
    let leading_issues: Vec<_> = issues
        .iter()
        .filter(|i| i.code == ValidationCode::LeadingSymbol)
        .collect();
...
fn title_period_dot_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::Period);
}
...
fn title_exclamation_detected() {
    ...
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::ExclamationQuestion);
}
...
fn title_newline_detected() {
    let issues = validate_scratchpad_title("タイトル\n改行");
    assert_eq!(issues.len(), 1);
    assert_eq!(issues[0].code, ValidationCode::Newline);
}
...
fn title_multiple_violations() {
    ...
    let codes: Vec<ValidationCode> = issues.iter().map(|i| i.code).collect();
    assert!(codes.contains(&ValidationCode::LeadingSymbol));
    assert!(codes.contains(&ValidationCode::Slash));
    assert!(codes.contains(&ValidationCode::Space));
    assert!(codes.contains(&ValidationCode::Period));
}
